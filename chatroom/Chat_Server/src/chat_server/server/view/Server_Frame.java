...
import chat_server.server.backstage.ServerClientTogether;
...
public class Server_Frame extends JFrame{
	...
	public Server_Frame() throws UnsupportedLookAndFeelException, ClassNotFoundException, InstantiationException, IllegalAccessException {
		super("聊天室server");
		...
		jb3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//显示系统信息
				ShowSystemMessage("系统通知："+message+"\r\n");
				server.Send_SystemMessage("系统通知："+message+"\r\n");
				jta2.setText("");
				//获取光标
				jta2.grabFocus();
			}
		...
		jb4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(i==-1){
					...
					//取出选中的用户，并取出其线程序
					ServerClientTogether scc = ServerThreadCollection.getServerContinueConnetClient(onLine[i]);
					scc.CloseThread();
				}
    ...
    public static void showMessage(Message mess){
    	if(mess.getMessageType().equals(MessageType.Common_Message_ToAll)){
    		jta1.append(mess.getSender()+"对所有聊天室成员说： "+mess.getContent()+"\r\n");

 ...
 */
public class ServerClientTogether implements Runnable{
	...
	//将客户端的Socket传入
	public ServerClientTogether(Socket s, String UserName){
		this.s = s;
				...
				if(mess.getMessageType().equals(MessageType.Common_Message_ToAll)){
					...
					for(int i=0;i<strings.length;i++){
						...
						if(!mess.getSender().equals(Name)){
							...
							mess.setGetter(Name);
							//获得其他服务器端与客户端通信的线程
							ServerClientTogether sccc = ServerThreadCollection.getServerContinueConnetClient(Name);
					...
					ServerClientTogether sccc = ServerThreadCollection.getServerContinueConnetClient(mess.getGetter());
					os = new ObjectOutputStream(sccc.s.getOutputStream());
					os.writeObject(mess);
	...
	public void UpdataOnline(){
		...
		for(int i=0;i<strings.length;i++){
			...
			try {
				//取出每个服务器端与客户端通信的线程
				ServerClientTogether scc = ServerThreadCollection.getServerContinueConnetClient(Getter);
				if(scc!=null){

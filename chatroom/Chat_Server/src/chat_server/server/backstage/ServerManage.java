...
public class ServerManage implements Runnable{
					...
					if(user.getType().equals(MessageType.UserLogin)){
						...
						if(server.Check_IsLogin(user)){
							...
							if(server.CheckLogin(user) && server.Update_IsLogin(user, 1)){//更改成登陆了
								...
								ServerClientTogether scc = new ServerClientTogether(s,user.getName());
								//添加入集合
	...
	public void CloseServer(){
		try {
			...
			for(int i=0;i<strings.length;i++){
				//获得其他服务器端与客户端通信的线程
				ServerClientTogether sccc = ServerThreadCollection.getServerContinueConnetClient(strings[i]);
				if(sccc!=null){
					sccc.CloseThread();
				}
			}

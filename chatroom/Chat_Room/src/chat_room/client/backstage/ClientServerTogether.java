...
import chat_room.client.tools.ClientPeopleCollectionManagement;
 ...
 */
public class ClientServerTogether implements Runnable {
    private ClientManage cm;    //后台处理对象
    private Client_Frame_Single client;//个人聊天界面对象
    private Client_Frame cf;//群聊界面对象
    boolean friend = false;

    public ClientServerTogether(ClientManage cm, Client_Frame cf) {
        this.cm = cm;
        this.cf = cf;
    }
            ...
            if (cm.IsConnect()) {
                ...
                if (mess != null) {
                    if (mess.getMessageType().equals(MessageType.Common_Message_ToAll)) {
                        ...
                        client = ClientPeopleCollectionManagement.getClientPerson(str);
                        String str2 = "是否同意陌生人单聊申请（请注意你的隐私不要泄露）";
                        int choice = 1;
                        if (!ClientPeopleCollectionManagement.isExist(str) || !ClientPeopleCollectionManagement.getClientPerson(str).friend) {
                        ...
                        if (ClientPeopleCollectionManagement.isExist(str2))
                            friend = false;
                        //如果没找到就创建一个个人聊天界面
                        if (client == null && friend) {
                            try {
                                client = new Client_Frame_Single(mess.getGetter(), mess.getSender(), cm, friend);
                            ...
                            //添加入管理个人聊天的界面集合
                            ClientPeopleCollectionManagement.addClientPersonCollection(str, client);
                            ClientPeopleCollectionManagement.addClientPersonCollection(str2, client);
    ...
    public void ClosePersonClient(String Name) {
        String[] strings;
        Set<String> set = ClientPeopleCollectionManagement.getClientPersonSet();
        for (String s : set) {
            strings = s.split(" ");
            if (strings[0].equals(Name)) {
                Client_Frame_Single cf = ClientPeopleCollectionManagement.getClientPerson(s);
                //关闭个人聊天窗口
                cf.dispose();
                //移除个人聊天窗口
                ClientPeopleCollectionManagement.removeClientPerson(s);
            }
        }
    }
